...
def main():
    ...
    if args.json:
        json_output = build_json_output(args, target, repo_name, branch, remote, has_unstaged, has_staged, tree_out, log, branches, contents if args.files else None)
...
def build_json_output(args, target, repo_name, branch, remote, has_unstaged, has_staged, tree_out, log, branches, contents):
    status = []
    if not has_unstaged and not has_staged:
        status.append("Working tree: clean")
    if has_unstaged:
        status.append(f"Unstaged changes: {has_unstaged.split(chr(10))[-1]}")
    if has_staged:
        status.append(f"Staged changes: {has_staged.split(chr(10))[-1]}")
    ...
    if branches:
        result["branches"] = [b.strip() for b in branches.split("\n") if b.strip()]
    if contents:
